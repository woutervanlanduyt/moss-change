...
```{r eval=FALSE}
# every species has one substrate
analyse_data_telfer_agg %>%
  count(phylum, taxon_id_parent, substrate) %>%
  filter(n > 1)
...
```{r}
analyse_data_telfer_agg %>%
  count(`n_hok_1980-1999`)

analyse_data_telfer_agg %>%
  count(`n_hok_2000-2019`)
...
```{r substrate-model}
m_substrate <- glm(cbind(`n_hok_2000-2019`,
                        `n_hok_tot_2000-2019` - `n_hok_2000-2019`) ~ 
                    qlogis((`n_hok_1980-1999`)/`n_hok_tot_1980-1999`)
                  + phylum 
                  + qlogis((`n_hok_1980-1999`)/`n_hok_tot_1980-1999`):phylum 
                  + substrate * phylum
                  , 
    family = "binomial", 
    data = analyse_data_telfer_agg)
...
summary(m_substrate)
anova(m_substrate, test = "Chisq")
```

```{r model-ellenberg}
m_ellenberg <- glm(cbind(`n_hok_2000-2019`,
                        `n_hok_tot_2000-2019` - `n_hok_2000-2019`) ~ 
                    qlogis((`n_hok_1980-1999`)/`n_hok_tot_1980-1999`)
                  + phylum 
                  + qlogis((`n_hok_1980-1999`)/`n_hok_tot_1980-1999`):phylum 
...
plotResiduals(m_substrate) 
plotResiduals(m_ellenberg)
```
...
calc_substrate <- function(model) {
  ...
  nd_r <- nd_r %>%
    filter(!(substrate == "Mossen" & phylum == "Bryophyta"))
...
calc_ellenberg <- function(ellenberg = "ell_r", model) {
