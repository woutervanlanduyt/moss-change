...
We modeled the per-species probability in the recent period (occupied 4 km x 4 km squares out of the total number of squares) through a logit-link as a function of logit-transformed baseline occurrence probability and Ellenberg values for nitrogen, light, temperature and moisture in interaction with phylum (either mosses or liverworts).
Substrate types grouped by phylum were included as random effects.
Preliminary model building showed that the data were overdispersed relative to a Binomial distribution.
Therefore, we modelled the data with a Beta-Binomial distribution.
This distribution has parameters for the number of trials (not estimated), the per-trial probability ($\mu$) and an overdispersion parameter ($\phi$) (both estimated).
...
\Sigma = \begin{bmatrix}\sigma^2_{0,j}&\sigma_{01,j}\\\sigma_{01,j}&\sigma^2_{1,j}\end{bmatrix} && \text{variance-covariance matrix}
\end{aligned}
$$
...
In a similar way, substrate effects are calculated from the posterior linear predictor, conditional on Ellenberg values fixed at their median values.
...
```{r full-model, eval=!file.exists(find_root_file("brmsfit_alt_telfer.rds", criterion=is_git_root))}
...
saveRDS(alt_telfer, file = find_root_file("brmsfit_alt_telfer.rds", 
                                          criterion=is_git_root))
```

```{r eval=file.exists(find_root_file("brmsfit_alt_telfer.rds", criterion=is_git_root))}
alt_telfer <- readRDS(file = find_root_file("brmsfit_alt_telfer.rds", 
                                            criterion=is_git_root))
```

```{r}
broom.mixed::tidy(alt_telfer) %>%
  kable()
```
...
ps <- posterior_samples(alt_telfer, 
                        pars = c("b_Intercept",
                                 "b_phylum",
...
```{r plot-odds-ratio-random-effects-substrate}
pp_summary %>%
  group_by(phylum) %>%
...
```{r plot-lineardifference-random-effects-substrate}
pp_summary_linear_diff %>%
  group_by(phylum) %>%
  mutate(substrate = reorder(substrate, median)) %>%
  ggplot(aes(x = substrate, y = median, colour = phylum)) + 
...
pp_light_or <- pp_ellenberg()
pp_moisture_or <- pp_ellenberg(ellenberg = "ell_f", 
                            ellenberg_name = "Moisture")
pp_nitrogen_or <- pp_ellenberg(ellenberg = "ell_n", 
                             ellenberg_name = "Nitrogen")
pp_temperature_or <- pp_ellenberg(ellenberg = "ell_t",
                                ellenberg_name = "Temperature")

pp_data_or <- bind_rows(
  pp_light_or, pp_moisture_or, pp_nitrogen_or, pp_temperature_or)


pp_light_ld <- pp_ellenberg(odds_ratio = FALSE)
pp_moisture_ld <- pp_ellenberg(ellenberg = "ell_f", 
                            ellenberg_name = "Moisture",
                            odds_ratio = FALSE)
pp_nitrogen_ld <- pp_ellenberg(ellenberg = "ell_n", 
                             ellenberg_name = "Nitrogen",
                             odds_ratio = FALSE)
pp_temperature_ld <- pp_ellenberg(ellenberg = "ell_t",
                                ellenberg_name = "Temperature",
                                odds_ratio = FALSE)

pp_data_ld <- bind_rows(
  pp_light_ld, pp_moisture_ld, pp_nitrogen_ld, pp_temperature_ld)


```


```{r plot-or-fixed-effects-ellenberg, out.width="100%", fig.height=150/25.4}
pp_data_or %>%
...
```{r plot-ld-fixed-effects-ellenberg, out.width="100%", fig.height=150/25.4}
pp_data_ld %>%
  ggplot(aes(x = ellenberg_values, y = median)) + 
  geom_hline(yintercept = 0) +
  geom_ribbon(aes(ymin = lwr, 
...
# Text for methods section in paper
...
To minimize the effect of badly prospected grid cells we only used grid cells (16km²) where minimal 40 species were found in both periods (Figure 1).
This resulted in 224 grids prospected in both periods.
In these selected grids 35648 bryophyte observations (expressed as unique species per year and per 16 km² grid) were done in the first period and 45236 in the second period.
To correct for potential changes in survey effort within those grids we used a relative change index as proposed by (Telfer, Preston, & Rothery, 2002), with slight modifications.
...
To summarise the changes in distribution area for all species, we performed a generalized linear regression of the logit transformed proportions  count of occupied 16 km² squares in the second survey (out of the total number of 16 km² squares considered, i.e. proportional data) as a function of the logit-transformed proportion in the first survey (baseline)  and covariates (phylum, substrate and Ellenberg values). 
Because the counts were overdispersed (variance larger than Binomial distribution), a quasi-binomial distribution was used to correct for overdispersion.
The proportions in the second survey are estimated using a logit-link and related to the Ellenberg values with the following model equation:
...
\end{aligned}
$$
A separate model was used to relate the proportions to substrate:
