...
First we need to define `Stan` functions for the beta-binomial distribution, because this distribution is not predefined in the `brms` package. 
...
full_model <- brm(n_hok_2000_2019 | vint(n_hok_tot_2000_2019) + weights(wei) ~ 
...
The following code chunk makes some functions available which are needed for later post-processing.

```{r expose-functions}
expose_functions(full_model, vectorize = TRUE)
...
We used leave-one-out cross-validation as criterion to choose among the candidate models.
...
full_model <- add_criterion(full_model, "loo", 
                            moment_match = FALSE, 
                            overwrite = FALSE)

model_1 <- add_criterion(model_1, "loo", 
                            moment_match = FALSE,
                            overwrite = FALSE)

model_2 <- add_criterion(model_2, "loo", 
                            moment_match = FALSE,
                            overwrite = FALSE)

loo1 <- loo(full_model, model_1, model_2) 
```
...
```{r model-diagnostics}
plot(loo1$loos$model_2, label_points = TRUE)
```


```{r pp-check}
pp_check(model_2, type = "bars")
```
...
```{r plot-epred, fig.cap='(ref:plot-epred)', fig.height = 175/25.4, warning=FALSE}
pep0 <- posterior_epred(model_2, 
                        re_formula = NA,
                        newdata = conditions0 %>% 
                          mutate(substrate = NA,
...
p2 <- conditional_effects(model_2, 
...
p3 <- conditional_effects(model_2, 
...
p4 <- conditional_effects(model_2, 
                          re_formula = NULL,
