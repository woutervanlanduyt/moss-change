...
Now the interaction `qlogis((`n_hok_1980-1999` + 1)/`n_hok_tot_1980-1999`):phylum` is removed, but the difference in AIC is only marginal so I would keep it in and for the paper not do model selection at all and argue that the interactions with phylum are required by design.

The figures below thus show the full model (including all interactions).

```{r}
# overriding the stepAIC
alt_telfer_opt <- alt_telfer
```
...
calc_substrate() %>%
  ...
  filter(`n_hok_1980-1999` %in% c(20, 100, 180)) %>%
  mutate(substrate = reorder(substrate, link_est_diff),
         `n_hok_1980-1999` = factor(`n_hok_1980-1999`)) %>%
...
```{r ell-r-diff}
calc_ellenberg(ellenberg = "ell_r") %>%
  as_tibble() %>%
  mutate(link_est_diff = link_est - qlogis(prop_baseline),
         link_lwr_diff = link_lwr - qlogis(prop_baseline),
         link_upr_diff = link_upr - qlogis(prop_baseline)) %>% 
  filter(`n_hok_1980-1999` %in% c(20, 100, 180)) %>%
  mutate(`n_hok_1980-1999` = factor(`n_hok_1980-1999`)) %>%
  ...
  geom_ribbon(aes(ymin = link_lwr_diff, 
                    ymax = link_upr_diff,
                    fill = `n_hok_1980-1999`),
              alpha = 0.3) +
  geom_line(aes(colour = `n_hok_1980-1999`)) +
  facet_grid(~phylum)
```
...
```{r ell-f-diff}
calc_ellenberg(ellenberg = "ell_f") %>%
  as_tibble() %>%
  mutate(link_est_diff = link_est - qlogis(prop_baseline),
         link_lwr_diff = link_lwr - qlogis(prop_baseline),
         link_upr_diff = link_upr - qlogis(prop_baseline)) %>% 
...
```{r ell-l-diff}
calc_ellenberg(ellenberg = "ell_l") %>%
  as_tibble() %>%
  mutate(link_est_diff = link_est - qlogis(prop_baseline),
         link_lwr_diff = link_lwr - qlogis(prop_baseline),
         link_upr_diff = link_upr - qlogis(prop_baseline)) %>% 
...
```{r ell-n-diff}
calc_ellenberg(ellenberg = "ell_n") %>%
  as_tibble() %>%
  mutate(link_est_diff = link_est - qlogis(prop_baseline),
         link_lwr_diff = link_lwr - qlogis(prop_baseline),
         link_upr_diff = link_upr - qlogis(prop_baseline)) %>% 
...
```{r ell-t-diff}
calc_ellenberg(ellenberg = "ell_t") %>%
  as_tibble() %>%
  mutate(link_est_diff = link_est - qlogis(prop_baseline),
         link_lwr_diff = link_lwr - qlogis(prop_baseline),
         link_upr_diff = link_upr - qlogis(prop_baseline)) %>% 
