...
We focuses model selection on the Ellenberg terms only. 
...
The simplified model was refit and compared to the full model using leave-one-out cross validation as explained in @Vehtari2017.
The simplified model was preferred over the full model when 
alt_telfer_2 is the better model 
...
loo1 <- loo(alt_telfer, alt_telfer_2, save_psis = TRUE) # alt_telfer_2 is the better model 
# the elpd_diff is more than 2 times larger than its standard erro 
```

```{r model-diagnostics, echo = TRUE}
plot(loo1$loos$alt_telfer, label_points = TRUE)
plot(loo1$loos$alt_telfer_2, label_points = TRUE)

alt_telfer_2$data[745,]
...
pp_substrate(model = alt_telfer_2, 
           odds_ratio = TRUE) %>%
  group_by(phylum) %>%
...
pp_substrate(model = alt_telfer_2, odds_ratio = FALSE) %>%
  group_by(phylum) %>%
  mutate(substrate = reorder(substrate, median)) %>%
  ggplot(aes(x = substrate, y = median, colour = phylum)) + 
...
pp_moisture_or <- pp_ellenberg(model = alt_telfer_2, 
                               ellenberg = "ell_f", 
                            ellenberg_name = "Moisture")
pp_temperature_or <- pp_ellenberg(model = alt_telfer_2, ellenberg = "ell_t",
...
pp_moisture_ld <- pp_ellenberg(model = alt_telfer_2,
                               ellenberg = "ell_f", 
                            ellenberg_name = "Moisture",
                            odds_ratio = FALSE)
pp_temperature_ld <- pp_ellenberg(model = alt_telfer_2,
                                  ellenberg = "ell_t",
                                ellenberg_name = "Temperature",
                                odds_ratio = FALSE)
