...
```{r}
pp_check(alt_telfer_3, type = "dens_overlay")
```
...
(ref:change-recording) Change in recording probability. The total number of occupied grid cells in both periods equals 228. Points: observed values. Lines with ribbons: expectations and 95% confidence bounds conditional on `r conditions1$cond__`. Black line: line of no change.


Figure \@ref(fig:change-recording)

```{r change-recording, fig.cap='(ref:change-recording)'}

p1 <- conditional_effects(alt_telfer_3, 
                    effects = "n_hok_1980_1999:phylum",
                    conditions = conditions1)

plot(p1, plot = FALSE, points = TRUE)[[1]] + 
  geom_abline() +
  labs(y = "Number of occupied grid cells (2000 - 2019)",
...
conditions2 <- create_conditions(
  x = alt_telfer_3,
  vars = c("n_hok_1980_1999", "ell_t")
)

```

(ref:plot-ell-f) Effect of Ellenberg moisture values on occurrence of mosses. Points: observed values. Line with ribbons: expectation and 95% confidence bounds conditional on `r conditions2$cond__`. No interaction effect was modeled.

Figure \@ref(fig:plot-ell-f)

```{r plot-ell-f, fig.cap = '(ref:plot-ell-f)'}
p2 <- conditional_effects(alt_telfer_3, 
                    effects = c("ell_f:phylum"),
                    conditions = conditions2)

plot(p2, plot = FALSE, points = TRUE)[[1]] + 
  scale_x_continuous(
    breaks = function(x) 
...
(ref:plot-ell-t) Effect of Ellenberg temperature values on occurrence of mosses. Points: observed values. Line with ribbons: expectation and 95% confidence bounds conditional on `r conditions3$cond__`.

Figure \@ref(fig:plot-ell-t)

```{r plot-ell-t, fig.cap = '(ref:plot-ell-t)'}
p3 <- conditional_effects(alt_telfer_3, 
                    effects = c("ell_t:phylum"),
                    conditions = conditions3)

plot(p3, plot = FALSE, points = TRUE)[[1]] + 
  scale_x_continuous(
    breaks = function(x) 
      unique(floor(pretty(seq(0, (max(x) + 1) * 1.1))))) +
  labs(y = "Number of occupied grid cells (2000 - 2019)",
...
conditions4 <- create_conditions(
  x = alt_telfer_3,
  vars = c("n_hok_1980_1999", "ell_f", "ell_t")
)
```

(ref:plot-substrate) Effect of substrate on occurrence of mosses. Points and ranges: expectation and 95% confidence interval conditional on `r conditions4$cond__`.

Figure \@ref(fig:plot-substrate)

```{r plot-substrate, fig.cap = '(ref:plot-substrate)'}
p4 <- conditional_effects(alt_telfer_3, 
                          re_formula = NULL,
                          effects = c("substrate:phylum"),
                          conditions = conditions4)

plot(p4, plot = FALSE, points = FALSE)[[1]] + 
...
### Y-scale = linear difference in logit scale
